...
\begin{document}
\title{{\fontspec{Copperplate Gothic Bold}Geometry Homework 1}}
...
\begin{enumerate}
\item[]
\item[(a)] Since $\alpha$ is a closed convex plane curve, by the theorem of turning tangents (P.37), we have \[ \int_0^l \kappa(s)ds = 2\pi \] Moreover, $\kappa(s)$ and $r$ are both positive by definition, so $r\kappa(s)/\|\alpha'(s)\|$ is always non-negative. Therefore, length of $\beta$ is 
...
\end{align*}
\item[(b)]
By using the result of (c), we knew that $\kappa_\beta$ is always positive, so $\beta$ is a convex closed curve and hence simple.
...
\\=& \left| A(\alpha) + \frac{r^2}2\oint_0^l \kappa ds + r\oint_0^l\|\alpha'\| ds \right| = A(\alpha) + \pi r^2 + rl.
\end{align*}
\item[(c)]
\begin{align*}
\beta'(s)&=\alpha'(s)-rn'(s)\\
...
%第十題
\begin{problem}
Let $F(x, y)=c$ define a plane curve. Prove that the curvature of the curve satisfies
\[
|\kappa| = \left|
\frac{
\left[
