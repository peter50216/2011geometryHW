...
\begin{align*}
\gamma(t) =& (x(s(t)), y(s(t))); \\
\text{Set }s(t) =& \arcosh(e^t). \\
\gamma'(t) =& (x'(s)s', y'(s)s'); \\
|\gamma'(t)|^2 =& x'(s)^2s'^2 + y'(s)^2s'^2 = s'^2[(1 - \sech^2s)^2 + \sech^2s\tanh^2s]; \\
	=& s'^2\tanh^2s = [(\pm 1) e^t/\sqrt{1-e^{2t}} \cdot \sqrt{e^{2t} - 1}/e^t]^2 = 1. \\
\text{Thus in $\gamma$ $t$ is arc-length.}
\end{align*}
\item[(b)] 
\end{enumerate}
\end{document}
